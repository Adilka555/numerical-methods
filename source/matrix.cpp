...
/*
Matrix Matrix::operator+(vector<double> a) {
  Matrix buf (m_rowSize, 0, 0.0);
  Matrix result (m_rowSize, m_colSize, 0.0);
  for (unsigned i = 0; i < m_rowSize; i++){
    buf(i,0) = a[i];
  }
  for (unsigned i = 0; i < m_rowSize; i++) {
      for (unsigned j = 0; j < m_colSize; j++) {
          result(i,j) = this->m_matrix[i][j] + buf(i,j);
      }
  }
  return result;
}
*/
vector<double>& Matrix::operator[](const unsigned &num) {
