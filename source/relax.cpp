...
auto relax (Matrix u, Matrix F, double h2,Matrix au, Matrix &new_u) {
	unsigned n = u.getRows();
	new_u = u;
	double c, d, a = 0.0, b = 0.0, t;
	double h = 1 / (n-1);
	double ro = (1 - sin(PI * h))/(1 + sin(PI*h));
	double w = 2 / (1 + sin(PI * h));
		for (unsigned i = 1; i < n-1; i++) {
			for (unsigned j = 1; j < n-1; j++) {
				new_u(i,j) = u(i,j) + 1.8*(new_u(i-1,j) + u(i+1,j) + new_u(i,j-1) + u(i,j+1) - 4*u(i,j) + h2*F(i,j))/4;
				t = new_u(i,j);
				c = abs(t - u(i,j));
				d = abs(t - au(i,j));
				a = max(c,a);
